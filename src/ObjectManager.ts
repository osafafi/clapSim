...
export class ObjectManager extends THREE.Object3D {
  director: Director;
  ...
  findNeighbors() {
    ...
    this.crowd.children.forEach((child) => {
      const individual = child as Person;

      this.crowd.children.forEach((otherChild) => {
        const fellowIndividual = otherChild as Person;
        if (individual === fellowIndividual) return; // Skip self

        const distance = individual.position.distanceTo(
          fellowIndividual.position
        );
  ...
  simulateClaps(elapsedTime: number) {
  ...
  convergeClap(src: number, dest: number) {
...
function generateRandomAttributes(): {
  ...
  const pastelColor = new THREE.Color(
    `hsl(${Math.random() * 360}, 100%, 85%)` // Hue, Saturation, Lightness
  );

  return { number: randomNumber, color: pastelColor, clapsPerSecond: cps };
}

function generateRandomePosition(bounds: number) {
  const position = new THREE.Vector3(
    Math.random() * 2 * bounds - bounds,
    1,
    Math.random() * 2 * bounds - bounds
  );
...
class Person extends THREE.Object3D {
  ...
  constructor(bounds: number) {
    ...
    this.label = new CSS2DObject(cpsDiv);
